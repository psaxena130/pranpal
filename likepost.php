...
$conn=new mysqli("localhost","root","","$post1");
$sql="INSERT INTO likes VALUES(\"$id\")";
if(!$conn->query($sql))
	echo($conn->error);
$conn=new mysqli("localhost","root","","$to");
$sql="UPDATE posts SET likes=likes+1 WHERE post_id=\"$post\"";
if(!$conn->query($sql))
	echo($conn->error);
$sql="INSERT INTO notification VALUES(\"3\",\"$id\")";
if(!$conn->query($sql))
	echo($conn->error);
$conn=new mysqli("localhost","root","","$id");
$sql="INSERT INTO liked VALUES(\"$post\",\"$to\")";
$conn->query($sql);
}
?>